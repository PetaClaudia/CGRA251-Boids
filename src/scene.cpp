...
void Scene::loadCompletion() {
    ...
    //had to make 50 in each flock, otherwise it lags badly
    m_boids.clear();
    for(int i = 0; i<=50; i++){
        m_boids.push_back(Boid(linearRand(vec3(-20), vec3(-20)), sphericalRand(1.0), 1));
    }
    for(int i = 0; i<=50; i++){
        m_boids.push_back(Boid(linearRand(vec3(-1), vec3(1)), sphericalRand(1.0), 2));
    }
    //preditor
    m_boids.push_back(Boid(linearRand(vec3(-1), vec3(1)), sphericalRand(1.0), 3));
}
...
void Scene::loadChallenge() {
    ...
    for(int i=0; i<4; i++){
        m_obstacles.push_back(Boid(linearRand(vec3(-1), vec3(1)), sphericalRand(1.0), 1));
    }
}
...
void Scene::renderGUI() {
    ...
    if (ImGui::Button("Core", ImVec2(80, 0))) { loadCore(); completion = false; challenge = false;}
	ImGui::SameLine();
    if (ImGui::Button("Completion", ImVec2(80, 0))) { loadCompletion(); completion = true; challenge = false;}
	ImGui::SameLine();
    if (ImGui::Button("Challenge", ImVec2(80, 0))) { loadChallenge(); completion = false; challenge = true;}
	...
	// YOUR CODE GOES HERE
	// ...
    ImGui::SliderFloat("Min Speed", &minSpeed, 0.0f, 5.0f);
    ImGui::SliderFloat("Max Speed", &maxSpeed, 10.0f, 20.0f);
    ImGui::SliderFloat("Max Acceleration", &speed, minSpeed, maxSpeed);
    ImGui::SliderFloat("Neighbourhood size", &neighbourDist, 0.0f, 100.0f);
    ImGui::SliderFloat("Cohesion", &cohesionWeight, 0.0f, 10.0f);
    ImGui::SliderFloat("Alignment", &alignmentWeight, 0.0f, 10.0f);
    ImGui::SliderFloat("Avoidance", &avoidanceWeight, 0.0f, 50.0f);
    if(completion == true){
        ImGui::SliderFloat("Predator Speed", &predatorSpeed, minSpeed, maxSpeed);
        ImGui::SliderFloat("Predator Neighbourhood size", &predatorNeighbourDist, 0.0f, 100.0f);
        ImGui::SliderFloat("Predator Avoidance", &predatorAvoidanceWeight, 0.0f, 50.0f);
        ImGui::SliderFloat("Predator Chase", &predatorChaseWeight, 0.0f, 100.0f);
        ImGui::SliderFloat("Anti-Predator weight", &antiPredatorWeight, 0.0f, 50.0f);
    }
}
